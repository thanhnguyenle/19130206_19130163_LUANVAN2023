...
public class ImageRepository implements IMaterialDBIRepository {
    ...
    private static final String CREATE_IMAGE = "INSERT INTO `image` VALUES (:url,:description,:materialID)";
    ...
    public List<MaterialEntities> filterAllMaterialByProductID(String productID, String searchType, String searchValue, String sortType, String sortValue) {
        ...
        if(productID != null && !productID.isEmpty()){
            query.append(" WHERE id IN (SELECT materialID FROM `material_product` WHERE productID = :productID)");
            haveProductID = true;
        }
        if(searchType != null && !searchType.isEmpty()){
            ...
            query.append(searchType).append(" LIKE %:searchValue%");
        }
    ...
    public List<MaterialEntities> filterAllMaterialBySupplierID(String supplierID, String searchType, String searchValue, String sortType, String sortValue) {
        ...
        if(supplierID != null && !supplierID.isEmpty()){
            query.append(" WHERE id IN (SELECT materialID FROM `material_supplier` WHERE supplierID = :supplierID)");
            haveSupplierID = true;
        }
        if(searchType != null && !searchType.isEmpty()){
            if(haveSupplierID){
    ...
    @Override
    public List<MaterialEntities> filterAllMaterial(IPaging paging, String searchType, String searchValue, String sortType, String sortValue) {
        List<MaterialEntities> materialEntities = filterAllMaterial(searchType, searchValue, sortType, sortValue);
        if(paging == null)
            return materialEntities;
        return materialEntities.stream().skip(paging.getOffset()).limit(paging.getLimit()).toList();
    }
    ...
    @Override
    public List<MaterialEntities> filterAllMaterial(String searchType, String searchValue, String sortType, String sortValue) {
        StringBuilder query = new StringBuilder(GET_LIST);
        if(searchType != null && !searchType.isEmpty()){
            query.append(" WHERE ").append(searchType).append(" LIKE %:searchValue%");
        }
        if(sortType != null && !sortType.isEmpty()){
            query.append(" ORDER BY ").append(sortType).append(" ").append(sortValue);
        }
        ...
        return jdbi.withHandle(handle -> {
            ...
            imageIDs.forEach(image -> {
                preparedBatch
                        ...
                        .bind("materialID", image.getMaterialID())
                        .add();
            });
            return preparedBatch.execute().length > 0;
        });
